    ...
    else if (auto mpduSubframe = dynamicPtrCast<const Ieee80211MpduSubframeHeader>(chunk))
    {
        ...
        switch (type) {
            ...
            case ST_ACTION: {
                ...
                stream.writeUint4(actionFrame->getSequenceNumber().getRaw());
                stream.writeNBitsOfUint64Be(actionFrame->getFragmentNumber(), 12);
                switch (actionFrame->getCategory()) {
                    case 1: {
                        throw cRuntimeError("Ieee80211MacHeaderSerializer: cannot serialize the Ieee80211ActionFrame frame, category 1 not supported.");
                        break;
                    }
                    case 2: {
                        throw cRuntimeError("Ieee80211MacHeaderSerializer: cannot serialize the Ieee80211ActionFrame frame, category 2 not supported.");
                        break;
                    }
                    case 3: {
...
const Ptr<Chunk> Ieee80211MacHeaderSerializer::deserialize(MemoryInputStream& stream) const
{
    /*if (B(stream.getRemainingLength()) == B(14)) {
        ...
        std::cout << "deserializing Ieee80211MsduSubframeHeader" << endl;
        return msduSubframe;
    }
        ...
        stream.readByte();
        stream.readByte();
        std::cout << "deserializing Ieee80211MpduSubframeHeader" << endl;
        return mpduSubframe;
    }
    else {*/
