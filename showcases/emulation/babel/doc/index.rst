...
Testing Routing Protocols in a Simulated Environment
====================================================
...
.. **TODO whats a network namespace**
...
.. Routes will change as connectivity changes between the three hosts.
...
.. **TODO this is the same TAP device as in the simulation**
...
.. After the simulation has concluded, the ``Teardown.sh`` script can be run to destroy
   the TAP interfaces and the network namespaces:

.. **TODO** The ``Teardown.sh`` script cn be run to destroy the TAP interfaces and the network namespaces
   when they are not needed anymore (the user has finished exploring the simulation)

   -the teardown script can destroy the namespaces when they are not needed
   -but it doesnt have to be run all the time
...
.. **V1** The configuration selects the TAP interfaces from network namespaces of the host OS,
   and copies their configuration, such as assigned IP addresses and MAC addresses. **TODO for each host/netns**

.. **V2** For each host, the configuration selects the corresponding network namespace and TAP device,
   and copies the TAP device's configuration, such as assigned IP addresses and MAC addresses.
...
.. - All three hosts are out of communication range (e.g. at the beginning)
   - The route between ``host[0]`` and ``host[2]`` goes through ``host[1]``
   - All hosts can reach each other directly
...
.. This Babel message broadcasts a route; it contains several `type-length-value`
   ...
   - It contains a ``hello`` message, which is multicast, and used to probe connectivity between the nodes.
   - The hello message is followed by an ``ihu`` ('I heard you'), which is a reply to a previously received hello message; it is multicast, but contains a target address. The `Rxcost` field indicates the link quality between the two nodes.
   - The ``nh`` (next hop) message is implied by the subsequent ``update`` message (or messages).
   - The ``router-id`` message identifies a node with a unique ID; it is implied by the subsequent ``update`` messages as well.
   - The ``update`` message contains an address and a metric.
   - The ``nh``, ``router-id`` and ``update`` TLVs describe a route; Here, these TLVs indicate that 192.168.4.1 (``host2``) can be reached via 192.168.3.1 (``host1``), in one hop (`metric=256`).

.. **V1** This Babel message broadcasts a route; it contains several `type-length-value`
   elements (TLVs). The ``nh`` and ``update`` messages indicate that the next hop is 192.168.3.1 (``host[1]``) for reaching 192.168.4.1 (``host[2]``).

.. **V2** This Babel message broadcasts a route; it contains several `type-length-value`
   elements (TLVs). The ``nh`` and ``update`` TLVs describe the route. They indicate that 192.168.4.1 (``host2``) can be reached via 192.168.3.1 (``host1``), in one hop (`metric=256`).
...
.. **TODO replace wireshark image when the addresses are fixed**
